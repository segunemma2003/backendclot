...
const ProductCategory: React.FC<Props> = ({
  ...
  return (
    <div>
      {/* ===== Head Section ===== */}
      <Header title={`${capitalizedCategory} - Henrimaris Fashion`} />
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  if (paramCategory !== "new-arrivals") {
    const numberOfProductsResponse = await axios.get(
      ...
      `https://server.henrimars.com/api/products?q=${paramCategory}`
    );
  ...
  const reqUrl =
    ...
    "https://server.henrimars.com/api/products":"https://server.henrimars.com/api/products";
  ...
  const fetchedProducts = res.data.data.map((product: apiProductsType) => ({
    ...product,
    img1: product.images!=null?product.images[0].image: "",
    img2: (product.images!=null && product.images[1]!=null)?product.images[1].image:product.images![0].image
  }));
