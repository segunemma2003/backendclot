...
const Home: React.FC<Props> = ({ products }) => {
  ...
  useEffect(() => {
    console.log(process.env.NEXT_PUBLIC_PROD_BACKEND_URL);
    if (!isFetching) return;
    const fetchData = async () => {
      const res = await axios.get(
        `https://server.henrimars.com/api/products`
      );
      console.log(res);
        ...
        <section className="w-full h-auto py-10 border border-b-2 border-gray100">
          <div className="app-max-width app-x-padding h-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="w-full sm:col-span-2 lg:col-span-2">
              <OverlayContainer
                imgSrc="/bg-img/Frame8.png"
                imgSrc2="/bg-img/Fram12.png"
                imgAlt="New Designs"
              >
                <LinkButton
