...
const Product: React.FC<Props> = ({ product, products }) => {
  ...
  return (
    <div>
      {/* ===== Head Section ===== */}
      <Header title={`${product.name} - Henrimaris Fashion`} />
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  const res = await axios.get(
    `https://server.henrimars.com/api/products/${paramId}`
  );
  ...
  let product: itemType = {
    ...
    img1:  fetchedProduct!.images![0]!.image,
    img2: "",
    // fetchedProduct!.images![1]!.image,
  ...
  const randomProductRes = await axios.get(
    `https://server.henrimars.com/api/products?q=${product.categoryName}`
  );
  ...
  let randomFetchedProducts = shuffled.slice(0,2);
  ...
  randomFetchedProducts.forEach((randomProduct: apiProductsType) => {
    products.push({
      ...
      img1: randomProduct.images![0]!.image,
      img2:  ""
      // randomProduct.images![1]!.image ?? "",
    });
  });
